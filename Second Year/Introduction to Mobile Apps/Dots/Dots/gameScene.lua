...
if composer.getVariable( "gridSize" ) ~= dots.LINES and composer.getVariable( "gridSize" ) ~= nil then
	LINES = composer.getVariable( "gridSize" )
	dots.LINES = composer.getVariable( "gridSize" )
else
	LINES = dots.LINES
end
local GAPS = LINES-1
...
local function handleNewGame( event )
	newGame()
end
...
local function updateScores()	
	...
	time = 400,
	params = {
		redScore = dots.players.red.score,
		yellowScore = dots.players.yellow.score
	}
}
	if dots:isOver() == true then
		composer.showOverlay( "gameEndOverlay", options )
	end
end
...
function scene:create( event )
    ...
    -- Code here runs when the scene is first created but has not yet appeared on screen
		...
		sceneGroup:insert( buttonNewGame )
